...
4. Phước bảo vệ tài khoản và khả năng truy vết: PAM, password policy, journald, rsyslog.
```
...
| 2 | `1.3.1.4` | Ensure the SELinux mode is not disabled | SELinux | Dùng `getenforce` hoặc `sestatus` |
...
| 7 | `6.2.3.2` | Ensure rsyslog service is enabled and active | Auditing | Kiểm tra `systemctl is-enabled/is-active rsyslog` |
...
Phước: Tài khoản và truy vết — PAM, password policy, journald, rsyslog.
```
...
| 2 | `1.3.1.4` | Ensure SELinux mode is not disabled | Đảm bảo SELinux không bị tắt |
...
```text
Phần M4 tập trung vào lớp nhận diện người dùng và khả năng truy vết của AlmaLinux 9 Server. Các tiêu chí được chọn liên quan đến PAM, chính sách mật khẩu, tài khoản root, journald và rsyslog. Đây là lớp bảo mật giúp kiểm soát đăng nhập, giảm rủi ro tài khoản yếu và hỗ trợ điều tra sự cố.
```
...
| 7 | `6.2.3.2` | Ensure rsyslog service is enabled and active | Đảm bảo rsyslog hoạt động để thu thập log hệ thống |
